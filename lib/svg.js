...
const orientPaths = (paths, approximation_error) => {
  const polygons = paths.map(getPolygonApproximation);
  for (var i = 0; i < paths.length; i++) {
    ...
    for (let j = 0; j < paths.length; j++) {
      if (j === i) {
        continue;
      } else if (polygonContainsPoint(polygons[j], path[0].start)) {
        contains += 1;
      }
    }
...
const convertCommandsToPath = (commands) => {
  const terms = [];
...
const getPolygonApproximation = (path, approximation_error) => {
  const result = [];
...
export {
  ...
  convertPathsToSVGPath,
  convertSVGPathToPaths,

...
const normalizeBounds = (bounds, max_ratio, min_width) => {
  ...
  if (diff[0] < min_width) {
    const extra = (min_width - diff[0]) / 2;
    bounds[0][0] -= extra;
    bounds[1][0] += extra;
  }
  if (diff[1] < min_width) {
    const extra = (min_width - diff[1]) / 2;
    bounds[0][1] -= extra;
    bounds[1][1] += extra;
  }
  if (max_ratio > 0) {
    ...
    if (diff[0] < diff[1]/max_ratio) {
      const extra = (diff[1]/max_ratio - diff[0]) / 2;
      bounds[0][0] -= extra;
      bounds[1][0] += extra;
    } else if (diff[1] < diff[0]/max_ratio) {
      const extra = (diff[0]/max_ratio - diff[1]) / 2;
      bounds[0][1] -= extra;
      bounds[1][1] += extra;
    }
  }
...
this.makemeahanzi.Matcher = class Matcher {
  constructor(medians, params) {
    ...
    params.min_width = coerce(params.min_width, 8);

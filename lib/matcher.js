...
const preprocessMedians = (medians, params) => {
  ...
  const source = params.bounds ? params.bounds :
      normalizeBounds(getBounds(medians), params.max_ratio, params.min_width);
...
this.makemeahanzi.Matcher = class Matcher {
  constructor(medians, params) {
    params = params || {};
    params.bounds = coerce(params.bounds, null);
  ...
  score(source, target) {
    ...
    return scoreMatch(source, target, this._params);
  }
}

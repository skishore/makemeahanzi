...
function run_classifier(features) {
  var distance_penalty = Math.pow(features[6]/MAX_BRIDGE_DISTANCE, 2);
  var score = -(Angle.penalty(features[0]) + Angle.penalty(features[1]));

  if (features[0] > 0 && features[1] > 0) {
    var alternate = -(Angle.penalty(Angle.subtract(features[0], Math.PI/2)) +
                      Angle.penalty(Angle.subtract(features[1], Math.PI/2)));
    score = Math.max(score, alternate - DEAD_END_PENALTY);
  }

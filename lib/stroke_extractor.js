...
function score_segments(ins, out) {
  ...
  var ains = Angle.subtract(angle, ins.angle);
  var aout = Angle.subtract(out.angle, angle);
  var threshold = Math.pow(MAX_BRIDGE_DISTANCE, 2);
  var distance = Point.distance2(out.point, ins.point)/threshold;
  var score = -(Angle.penalty(ains) + Angle.penalty(aout));

  if (ains > 0 && aout > 0) {
    var alternate = -(Angle.penalty(Angle.subtract(ains, Math.PI/2)) +
                      Angle.penalty(Angle.subtract(aout, Math.PI/2)));
    score = Math.max(score, alternate);
  }

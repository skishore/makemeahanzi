...
let cached_medians_promise = null;
...
const getMediansPromise = () => {
  if (!cached_medians_promise) {
    cached_medians_promise =
        loadBinaryData('medians.bin').then(decodeMedians)
                                     .catch(console.err);
  }
  return cached_medians_promise;
}

export {getMediansPromise};
